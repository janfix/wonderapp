    ...
    */
    ...
    // All items element must retreive their position  & their Params! 
        ...
        for (let i = 0; i < oldChoices.length; i++) {
            if(typeof allIGroups[i] !== "undefined"){
    ...
    for (let i = 0; i < item.choices.length; i++) {
        if(typeof allNGroups[i] !== "undefined"){
            ...
            if (item.choices[i].rightAnswer) {
                ...
                allNGroups[i].findOne(".scoreDot").setAttr("type","Right");
            }
            if (item.choices[i].eliminate) {
                //console.log("eliminate")
                allNGroups[i].findOne(".scoreDot").visible(true);
                allNGroups[i].findOne(".scoreDot").fill("#FFA500");
                allNGroups[i].findOne(".scoreDot").setAttr("type","Elimination");
            }
            if (item.choices[i].penalty) {
                //console.log("Penalty")
                allNGroups[i].findOne(".scoreDot").visible(true);
                allNGroups[i].findOne(".scoreDot").fill("red");
                allNGroups[i].findOne(".scoreDot").setAttr("type","Penalty");
            }
    ...
    if(typeof item.param.metadata !== "undefined"){
        ...
        $("#metaSkill").val(item.param.metadata.skillfrm);
        $("#metaTask").val(item.param.metadata.taskdef);
        $("#metakeywords").val(item.param.metadata.keywords);
        $("#metaAge").val(item.param.metadata.age);
        $("#metaISCED").val(item.param.metadata.isced);
        $("#metaCtool").val(item.param.metadata.comptool);
        $("#metaResponse").val(item.param.metadata.responseformat);
        $("#metaCorrType").val(item.param.metadata.corrtype);
        $("#metaCorrGuide").val(item.param.metadata.corrguide);
        $("#metaTextLink").val(item.param.metadata.textversion);
    ...
    questionNode.text(item.question); // Necessary in case of parser ->simple json not ok
